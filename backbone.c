...
void backbone(void)
{
  ...
  PreBaseLineNum = MAX_OF(0,PreBaseLineNum);
  StimNum = MAX_OF(0,StimNum);
  InterStimNum = MAX_OF(0,InterStimNum);
  NEpochs = MAX_OF(1,NEpochs);
  PostBaseLineNum = InterStimNum - PreBaseLineNum;
  PostBaseLineNum = MAX_OF(0,PostBaseLineNum);
  /* PVM_NRepetitions = PreBaseLineNum + StimNum*NEpochs + InterStimNum*(NEpochs-1) + PostBaseLineNum; */
  PVM_NMovieFrames = PreBaseLineNum + StimNum*NEpochs + InterStimNum*(NEpochs-1) + PostBaseLineNum; 
  /* PVM_NRepetitions = StimNum*NEpochs + InterStimNum*NEpochs; */
  /* We are providing a fast demo script here for testing,  need to be improved, wait for Beta version. always keep PreBaseLineNum=0 and PostBaseLineNum=0 */
   /* The NR should be default with 1 */
   PVM_NMovieFrames = MAX_OF(1,PVM_NMovieFrames); 
